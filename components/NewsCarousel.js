...
import { SafeAreaView } from 'react-native-safe-area-context'; // ✅ To'g'ri import
...
export default function NewsCarousel() {
  ...
  const infiniteData = useMemo(() => {
    ...
    return Array.from(
      { length: baseData.length * MULTIPLIER },
  ...
  if (loading) {
    return (
      <View style={[styles.carouselContainer, styles.center]}>
        <ActivityIndicator />
      </View>
    );
  }
  if (!news.length) {
    return (
      <View style={[styles.carouselContainer, styles.center]}>
        <Text style={{ color: '#6b7280' }}>Not available news</Text>
      </View>
    );
    }
